...
**Model:** native startup veil. During initial session/library preparation the **whole window** becomes a frosted, behind-window Liquid-Glass surface - you see straight through it to the **desktop and other windows** (the app shell is not drawn behind it) - with a small animated mark centered. When the library is ready it **quickly fades into the real library window**.
...
| Small, subtle, semi-transparent, animated, original mark | `LoadingMark`: a 2×2 lattice of rounded photo-tile outlines pulsing `0.28…0.78` in a `sin()` wave (`TimelineView(.animation)`), monochrome/vibrant, abstract - not Proton/Apple-Photos/camera/cloud/lock. |
