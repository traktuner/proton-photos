...
enum AccountDataCache {
    private static func dir(in baseDirectory: URL) -> URL {
        try? FileManager.default.createDirectory(at: baseDirectory, withIntermediateDirectories: true)
        return baseDirectory
    }

    private static func file(uid: String, kind: String, in baseDirectory: URL) -> URL {
        dir(in: baseDirectory).appendingPathComponent("account-\(kind)-\(uid).enc")
    }
    ...
    static func save(users: Data, addresses: Data, uid: String, keyPassword: String, in baseDirectory: URL) {
        let k = key(uid: uid, keyPassword: keyPassword)
        seal(users, to: file(uid: uid, kind: "users", in: baseDirectory), using: k)
        seal(addresses, to: file(uid: uid, kind: "addresses", in: baseDirectory), using: k)
    }

    /// The cached raw JSON, or nil if absent / undecryptable (wrong key / corruption / missing).
    static func load(uid: String, keyPassword: String, in baseDirectory: URL) -> (users: Data, addresses: Data)? {
        let k = key(uid: uid, keyPassword: keyPassword)
        guard let u = open(file(uid: uid, kind: "users", in: baseDirectory), using: k),
              let a = open(file(uid: uid, kind: "addresses", in: baseDirectory), using: k) else { return nil }
        return (u, a)
    }

    /// Erases the cached account blobs (wired to sign-out + "delete cache").
    static func clear(uid: String, in baseDirectory: URL) {
        try? FileManager.default.removeItem(at: file(uid: uid, kind: "users", in: baseDirectory))
        try? FileManager.default.removeItem(at: file(uid: uid, kind: "addresses", in: baseDirectory))
    }
